...
// 2. Inventory Valuation (detailed)
export const getInventoryValuation = async (req, res) => {
  try {
    ...
    const mostValuable = listings.reduce((max, prod) => (prod.quantity * prod.price > (max?.quantity || 0) * (max?.price || 0) ? prod : max), listings[0] || null);
    const soonestToExpire = listings.filter(l => l.expireDate).sort((a, b) => new Date(a.expireDate) - new Date(b.expireDate))[0] || null;
    res.json({
      totalValue,
      totalItems,
      totalQuantity,
...
// 4. Export Inventory as CSV
export const exportInventoryCSV = async (req, res) => {
  try {
    const listings = await ProductListing.find();
    const headers = [
      'Waste Category','Waste Type','Waste Item','Province','District','City','Quantity','Price','Description','Expire Date','Status','Farmer ID','Bank Name','Account Number','Account Holder Name','Branch'
    ];
    const rows = listings.map(l => [
      l.wasteCategory, l.wasteType, l.wasteItem, l.province, l.district, l.city, l.quantity, l.price, l.description, l.expireDate ? l.expireDate.toISOString().slice(0, 10) : '', l.status, l.farmerId, l.bankName, l.accountNumber, l.accountHolderName, l.branch
    ]);
...
export const exportInventoryPDF = async (req, res) => {
  try {
    ...
    doc.moveDown();
    const headers = ['Waste Category','Waste Type','Waste Item','Province','District','City','Quantity','Price','Expire Date','Status'];
    doc.fontSize(12);
    doc.text(headers.join(' | '));
    doc.moveDown(0.5);
    listings.forEach(l => {
      doc.text([
        l.wasteCategory, l.wasteType, l.wasteItem, l.province, l.district, l.city, l.quantity, l.price, l.expireDate ? l.expireDate.toISOString().slice(0, 10) : '', l.status
      ].join(' | '));
    });
    doc.end();
  } catch (err) {
